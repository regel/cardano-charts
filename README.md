...
1. Setup Azure storage account and init Terraform
1. Run Terraform scripts in `tf` folder to create all Azure resources
1. Write keys in the Vault: `coldSkey`, `coldVkey`, `coldCounter`, `kesSkey`, `vrfSkey`, `nodeCert`
1. Delete the `producer` pod to force the CSI driver to remount Vault volume
...
This will run for a couple of minutes and create all Azure resources (Vault, cluster, subnets, managed identities, network acls, etc) required to run a full Cardano node.

A new Azure Keyvault will be created empty. Secrets (hot and cold keys) will have to be set separately eg. by using the Azure Portal, or the az CLI.

The Terraform config files assume the existence of the `Cardano Admins` group in Active Directory. This AAD group will be permitted to manage the cluster and also manage the Vault. Creating (and rotating) secrets in this Key Vault is a manual step and should be performed by users in this AAD group. Group name can be changed in TF vars.
